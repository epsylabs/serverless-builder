...
class Scriptable(Generic):
    ...
    def enable(self, service):
        ...
        if self.inject_to_package:
            service.custom["scriptHooks"]["before:package:createDeploymentArtifacts"] = f"cp -a {self.inject_to_package} ."
            service.custom["scriptHooks"]["after:package:createDeploymentArtifacts"] = f"rm -rf ./{basename(self.inject_to_package)}"
